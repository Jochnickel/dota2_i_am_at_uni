...
function GameMode:DamageFilter(event)
	...
	if (RollPercentage(80)) then
		local randAbil = victimUnit:GetAbilityByIndex(RandomInt(0,29))
		if (randAbil) then
			victimUnit:SetCursorPosition(attackerUnit:GetAbsOrigin())
			victimUnit:SetCursorCastTarget(attackerUnit)
			randAbil:CastAbility()
		end
